...
const uniprot_accessions = CORUM
    .map(e => e['subunits(UniProt IDs)'].split(';').map(accession => {return {search_item: accession.toLocaleLowerCase(), id: e['ComplexID']}}))
    .reduce((current, previous) => previous.concat(current), []);

const gene_names = CORUM
    .map(e => e['subunits(Gene name)'].split(';').map(gene => {return {search_item: gene, id: e['ComplexID']}}))
    .reduce((current, previous) => previous.concat(current), []);
